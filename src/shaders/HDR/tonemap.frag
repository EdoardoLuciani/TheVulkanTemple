...
void main() {
    vec3 color = texture( render_target, fs_in.tex_coord ).rgb;
    ...
    // Using the new luminance, convert back to RGB
    vec3 out_color = xyY_to_rgb(xyY);

    // Convert back to sRGB and send to output buffer
    frag_color = vec4(pow(out_color, vec3(1.0/2.2)),1.0f);
