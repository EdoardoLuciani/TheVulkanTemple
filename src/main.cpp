...
private:
	...
	vulkan_helper::model_data_info water_bottle_model_data, box_model_data, table_model_data, fish_model_data, crow_model_data, sphere_model_data;
	int area_tex_size, search_tex_size;

	VkBuffer device_vertex_buffer, device_uniform_buffer;
	VkImage device_water_bottle_image, device_box_image, device_table_image, device_fish_image, device_crow_image, device_smaa_area_image, device_smaa_search_image;
	VkImageView device_water_bottle_color_image_view, device_water_bottle_orm_image_view,
		...
		device_table_color_image_view, device_table_orm_image_view, device_table_normal_image_view, device_table_emissive_image_view,
		device_fish_color_image_view, device_fish_orm_image_view, device_fish_normal_image_view, device_fish_emissive_image_view,
		device_crow_color_image_view, device_crow_orm_image_view, device_crow_normal_image_view, device_crow_emissive_image_view,
		device_smaa_area_image_view, device_smaa_search_image_view;
	VkSampler device_max_aniso_linear_sampler;
	std::array<VkMemoryRequirements, 9> device_memory_requirements;
	...
	std::array<VkDescriptorSet, 5> pbr_descriptor_sets;
	...
	glm::mat4 water_bottle_m_matrix, water_bottle_normal_matrix, box_m_matrix, box_normal_matrix, table_m_matrix, table_normal_matrix,
		fish_m_matrix, fish_normal_matrix, crow_m_matrix, crow_normal_matrix;
...
void VulkanSSAO::create_host_buffers() {
	...
	tinygltf::Model water_bottle, box, table, fish, crow, sphere;
	std::string err;
	std::string warn;
	...
	VkBufferCreateInfo buffer_create_info = {
		...
		vulkan_helper::get_model_data_total_size(fish_model_data) +
		vulkan_helper::get_model_data_total_size(crow_model_data) +
	...
	//	[---------------]-------[---------------]-------[---------------]-------
	// 	fish descriptor			crow descriptor
	// 	[---------------]-------[---------------]-------
	// 	[ set:0	128b   	] 128b	[ set:0	128b   	] 128b
	// 	[ 2*mat4		]		[ 2*mat4		]	
	// 	[---------------]-------[---------------]-------
	...
	buffer_create_info.size = 256*7+16;
	...
	fish_model_data.host_image_data_offset += offset;
	...
	vulkan_helper::copy_gltf_contents(crow,
		...
		vulkan_helper::t_model_attributes::T_NORMAL_MAP,
		vulkan_helper::t_model_attributes::T_EMISSIVE_MAP},
		static_cast<uint8_t*>(host_data_pointer) + offset, crow_model_data);
	crow_model_data.host_interleaved_vertex_data_offset += offset;
	crow_model_data.host_index_data_offset += offset;
	crow_model_data.host_image_data_offset += offset;
...
void VulkanSSAO::create_device_buffers() {
	...
	VkBufferCreateInfo buffer_create_info = {
		...
		fish_model_data.interleaved_vertex_data_size + fish_model_data.index_data_size +
		crow_model_data.interleaved_vertex_data_size + crow_model_data.index_data_size +
	...
	image_create_info.extent = fish_model_data.image_size;
	image_create_info.arrayLayers = fish_model_data.image_layers;
	vkCreateImage(device, &image_create_info, nullptr, &device_fish_image);

	image_create_info.extent = crow_model_data.image_size;
	image_create_info.arrayLayers = crow_model_data.image_layers;
	vkCreateImage(device, &image_create_info, nullptr, &device_crow_image);
	...
	vkGetImageMemoryRequirements(device, device_fish_image, &device_memory_requirements[5]);
	vkGetImageMemoryRequirements(device, device_crow_image, &device_memory_requirements[6]);
	...
	// Fish image view creation
	image_view_create_info.image = device_fish_image;
	image_view_create_info.format = VK_FORMAT_R8G8B8A8_SRGB;
	image_view_create_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 0, 1};
	vkCreateImageView(device, &image_view_create_info, nullptr, &device_fish_color_image_view);

	image_view_create_info.format = VK_FORMAT_R8G8B8A8_UNORM;
	image_view_create_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 1, 1};
	vkCreateImageView(device, &image_view_create_info, nullptr, &device_fish_orm_image_view);

	image_view_create_info.format = VK_FORMAT_R8G8B8A8_UNORM;
	image_view_create_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 2, 1};
	vkCreateImageView(device, &image_view_create_info, nullptr, &device_fish_normal_image_view);

	image_view_create_info.format = VK_FORMAT_R8G8B8A8_UNORM;
	image_view_create_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 3, 1};
	vkCreateImageView(device, &image_view_create_info, nullptr, &device_fish_emissive_image_view);

	// Crow image view creation
	image_view_create_info.image = device_crow_image;
	image_view_create_info.format = VK_FORMAT_R8G8B8A8_SRGB;
	image_view_create_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 0, 1};
	vkCreateImageView(device, &image_view_create_info, nullptr, &device_crow_color_image_view);

	image_view_create_info.format = VK_FORMAT_R8G8B8A8_UNORM;
	image_view_create_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 1, 1};
	vkCreateImageView(device, &image_view_create_info, nullptr, &device_crow_orm_image_view);

	image_view_create_info.format = VK_FORMAT_R8G8B8A8_UNORM;
	image_view_create_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 2, 1};
	vkCreateImageView(device, &image_view_create_info, nullptr, &device_crow_normal_image_view);

	image_view_create_info.format = VK_FORMAT_R8G8B8A8_UNORM;
	image_view_create_info.subresourceRange = {VK_IMAGE_ASPECT_COLOR_BIT, 0, VK_REMAINING_MIP_LEVELS, 3, 1};
	vkCreateImageView(device, &image_view_create_info, nullptr, &device_crow_emissive_image_view);
...
void VulkanSSAO::allocate_pbr_descriptor_sets() {
	std::array<VkDescriptorSetLayout, 5> layouts_of_sets;
	layouts_of_sets.fill(pbr_descriptor_sets_layout[0]);
	VkDescriptorSetAllocateInfo descriptor_set_allocate_info = {
		VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
		nullptr,
		descriptor_pool,
	...
	VkDescriptorBufferInfo fish_descriptor_buffer_info = { device_uniform_buffer, 768, 2 * sizeof(glm::mat4) };
	VkDescriptorImageInfo fish_descriptor_images_info[] = {
		{ device_max_aniso_linear_sampler, device_fish_color_image_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
		{ device_max_aniso_linear_sampler, device_fish_orm_image_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
		{ device_max_aniso_linear_sampler, device_fish_normal_image_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
		{ device_max_aniso_linear_sampler, device_fish_emissive_image_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL }
	};

	VkDescriptorBufferInfo crow_descriptor_buffer_info = { device_uniform_buffer, 1024, 2 * sizeof(glm::mat4) };
	VkDescriptorImageInfo crow_descriptor_images_info[] = {
		{ device_max_aniso_linear_sampler, device_crow_color_image_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
		{ device_max_aniso_linear_sampler, device_crow_orm_image_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
		{ device_max_aniso_linear_sampler, device_crow_normal_image_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
		{ device_max_aniso_linear_sampler, device_crow_emissive_image_view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL }
	};
	...
	},
	{
		VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
		nullptr,
		pbr_descriptor_sets[3],
		0,
		0,
		1,
		VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
		nullptr,
		&fish_descriptor_buffer_info,
		nullptr
	},
	{
		VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
	...
	},
	{
		VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
		nullptr,
		pbr_descriptor_sets[4],
		0,
		0,
		1,
		VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
		nullptr,
		&crow_descriptor_buffer_info,
		nullptr
	},
	{
		VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
	...
	vkUpdateDescriptorSets(device, 15, write_descriptor_set, 0, nullptr);
}
...
void VulkanSSAO::allocate_smaa_descriptor_sets() {
	...
	VkDescriptorBufferInfo smaa_rt_metrics = {device_uniform_buffer, 1792, sizeof(glm::vec4)};
...
void VulkanSSAO::upload_input_data() {
	...
	std::array <VkBufferCopy, 6> buffer_copies { {
	{ 
		0,0,
		water_bottle_model_data.interleaved_vertex_data_size + water_bottle_model_data.index_data_size
	},
	...
	{
		vulkan_helper::get_model_data_total_size(water_bottle_model_data) + vulkan_helper::get_model_data_total_size(box_model_data)
		+ vulkan_helper::get_model_data_total_size(table_model_data) + vulkan_helper::get_model_data_total_size(fish_model_data),
		water_bottle_model_data.interleaved_vertex_data_size + water_bottle_model_data.index_data_size +
		box_model_data.interleaved_vertex_data_size + box_model_data.index_data_size +
		table_model_data.interleaved_vertex_data_size + table_model_data.index_data_size +
		fish_model_data.interleaved_vertex_data_size + fish_model_data.index_data_size,
	...
	VkImageMemoryBarrier image_memory_barrier[7] = { {
	...
	vkCmdPipelineBarrier(command_buffers[0], VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 7, image_memory_barrier);
	...
	buffer_image_copy = {
		fish_model_data.host_image_data_offset,
		0,
		0,
		{VK_IMAGE_ASPECT_COLOR_BIT,0,0,4},
		{0,0,0},
		fish_model_data.image_size
	};
	vkCmdCopyBufferToImage(command_buffers[0], host_vertex_buffer, device_fish_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &buffer_image_copy);

	buffer_image_copy = {
		crow_model_data.host_image_data_offset,
		0,
		0,
		{VK_IMAGE_ASPECT_COLOR_BIT,0,0,4},
		{0,0,0},
		crow_model_data.image_size
	};
	vkCmdCopyBufferToImage(command_buffers[0], host_vertex_buffer, device_crow_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &buffer_image_copy);
	...
	vkCmdPipelineBarrier(command_buffers[0], VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 7, image_memory_barrier);
...
void VulkanSSAO::record_command_buffers() {
	...
	for (uint32_t i = 0; i < swapchain_images_count; i++) {
		...
		VkBufferCopy buffer_copy = { 0,0,256*7+16};
		...
		vkCmdBindDescriptorSets(command_buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, shadow_map_pipeline_layout, 0, 1, &pbr_descriptor_sets[3], 0, nullptr);
		offset = fish_model_data.device_interleaved_vertex_data_offset;
		vkCmdBindVertexBuffers(command_buffers[i], 0, 1, &device_vertex_buffer, &offset);
		vkCmdBindIndexBuffer(command_buffers[i], device_vertex_buffer, fish_model_data.device_index_data_offset, VK_INDEX_TYPE_UINT16);
		vkCmdDrawIndexed(command_buffers[i], fish_model_data.indices, 1, 0, 0, 0);

		vkCmdBindDescriptorSets(command_buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, shadow_map_pipeline_layout, 0, 1, &pbr_descriptor_sets[4], 0, nullptr);
		offset = crow_model_data.device_interleaved_vertex_data_offset;
		vkCmdBindVertexBuffers(command_buffers[i], 0, 1, &device_vertex_buffer, &offset);
		vkCmdBindIndexBuffer(command_buffers[i], device_vertex_buffer, crow_model_data.device_index_data_offset, VK_INDEX_TYPE_UINT16);
		vkCmdDrawIndexed(command_buffers[i], crow_model_data.indices, 1, 0, 0, 0);
		...
		vkCmdBindDescriptorSets(command_buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pbr_pipeline_layout, 0, 1, &pbr_descriptor_sets[3], 0, nullptr);
		offset = fish_model_data.device_interleaved_vertex_data_offset;
		vkCmdBindVertexBuffers(command_buffers[i], 0, 1, &device_vertex_buffer, &offset);
		vkCmdBindIndexBuffer(command_buffers[i], device_vertex_buffer, fish_model_data.device_index_data_offset, VK_INDEX_TYPE_UINT16);
		vkCmdDrawIndexed(command_buffers[i], fish_model_data.indices, 1, 0, 0, 0);

		vkCmdBindDescriptorSets(command_buffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pbr_pipeline_layout, 0, 1, &pbr_descriptor_sets[4], 0, nullptr);
		offset = crow_model_data.device_interleaved_vertex_data_offset;
		vkCmdBindVertexBuffers(command_buffers[i], 0, 1, &device_vertex_buffer, &offset);
		vkCmdBindIndexBuffer(command_buffers[i], device_vertex_buffer, crow_model_data.device_index_data_offset, VK_INDEX_TYPE_UINT16);
		vkCmdDrawIndexed(command_buffers[i], crow_model_data.indices, 1, 0, 0, 0);
...
void VulkanSSAO::frame_loop() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		fish_m_matrix = glm::translate(glm::vec3(-0.3f, -0.3f, 0.3f))*glm::rotate(glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f))
			*glm::scale(glm::vec3(0.3f,0.3f,0.3f));
		fish_normal_matrix = glm::transpose(glm::inverse(fish_m_matrix));

		crow_m_matrix = glm::translate(glm::vec3(0.4f, -0.18f, 0.45f))*glm::rotate(glm::radians(-45.0f), glm::vec3(0.0f, 1.0f, 0.0f))
			*glm::scale(glm::vec3(0.3f,0.3f,0.3f));
		crow_normal_matrix = glm::transpose(glm::inverse(crow_m_matrix));
		...
		// descriptor copy for fish
		dst_ptr += 256;
		memcpy(dst_ptr, glm::value_ptr(fish_m_matrix), sizeof(glm::mat4));
		memcpy(dst_ptr + sizeof(glm::mat4), glm::value_ptr(fish_normal_matrix), sizeof(glm::mat4));

		// descriptor copy for crow
		dst_ptr += 256;
		memcpy(dst_ptr, glm::value_ptr(crow_m_matrix), sizeof(glm::mat4));
		memcpy(dst_ptr + sizeof(glm::mat4), glm::value_ptr(crow_normal_matrix), sizeof(glm::mat4));

		// descriptor copy for camera
		dst_ptr += 256;
		...
		// descriptor copy for smaa_rt_metrics
		dst_ptr += 256;
...
VulkanSSAO::~VulkanSSAO() {
	...
	vkDestroyImageView(device, device_fish_color_image_view, nullptr);
	vkDestroyImageView(device, device_fish_orm_image_view, nullptr);
	vkDestroyImageView(device, device_fish_normal_image_view, nullptr);
	vkDestroyImageView(device, device_fish_emissive_image_view, nullptr);
	vkDestroyImage(device, device_fish_image, nullptr);

	vkDestroyImageView(device, device_crow_color_image_view, nullptr);
	vkDestroyImageView(device, device_crow_orm_image_view, nullptr);
	vkDestroyImageView(device, device_crow_normal_image_view, nullptr);
	vkDestroyImageView(device, device_crow_emissive_image_view, nullptr);
	vkDestroyImage(device, device_crow_image, nullptr);
